...
class Workflow implements WorkflowInterface
{
    ...
    private function leave( object $subject, Transition $transition, array $supress )
    {
        ...
        $this->dispatcher->dispatch($event, self::LEAVE_EVENT);

        // dispatch leave event for named workflow
        $this->dispatcher->dispatch($event, 'workflow.'. $this->name .'.leave');

        // dispatch leave event for left workflow state
        foreach ($states as $state) {
            $this->dispatcher->dispatch($event, 'workflow.'. $this->name .'.leave.'. $state);
        }
    }
    ...
    private function transition( object $subject, Transition $transition, array $supress )
    {
        ...
        // build event
        $event = new TransitionEvent($subject, $transition, $this);

        // dispatch general transition event
        $this->dispatcher->dispatch($event, self::TRANSITION_EVENT);

        // dispatch transition event for named workflow
        $this->dispatcher->dispatch($event, 'workflow.'. $this->name .'.transition');

        // dispatch transition event for transiton name
        $this->dispatcher->dispatch($event, 'workflow.'. $this->name .'.transition.' . $transition->getName());
    ...
    private function enter( object $subject, ?Transition $transition, array $supress )
    {
        ...
        $this->dispatcher->dispatch($event, self::ENTER_EVENT);

        // dispatch enter event for named workflow
        $this->dispatcher->dispatch($event, 'workflow.'. $this->name .'.enter');
        ...
        foreach ($states as $state) {
            $this->dispatcher->dispatch($event, 'workflow.'. $this->name .'.enter.' . $state);
        }
    }
    ...
    private function entered( object $subject, Transition $transition, array $supress )
    {
        ...
        $this->dispatcher->dispatch($event, self::ENTERED_EVENT);

        // dispatch entered event for named workflow
        $this->dispatcher->dispatch($event, 'workflow.'. $this->name .'.entered');
        ...
        foreach ($states as $state) {
            $this->dispatcher->dispatch($event, 'workflow.'. $this->name .'.entered.' . $state);
        }
    }
    ...
    private function registerListeners( $listeners )
    {
        ...
        foreach ($listeners as $listener) {
            $this->dispatcher->addListener($listener->getEvent(), $listener->execute());        
        }
    }
